...
# 1) Parkok és managerek elérhetősége (a) összes email, (b) részletes kapcsolati adatok (park neve, park email, manager neve, manager email, manager telefon)
...
# 3) Fő adatok összehasonlítása régiónként vagy vármegyékként, valamint éves összehasonlítás
...
def load_data(db):
    ...
    try: 
        ...
        eu = pd.read_sql("SELECT * FROM EU_tamogatas_latest", conn)
        kapcs = pd.read_sql("SELECT * FROM kapcsolatok_latest", conn)
...
# Egy adatoszlop adatainak összehasonlítása több park, több év viszonylatában, különbséggel
...
# EGY régió/park összes mutatójának idősoros elemzése.
def compare_single_entity_all_metrics(df, group_col, entity_name, agg_dict, years=None):
    """
    Egyetlen park/vármegye/régió mélyelemzése.
    ...
    if len(years_present) >= 2:
        first_y = years_present[0]
        last_y = years_present[-1]

        # Abszolút különbség az első és utolsó év között
        report['Teljes változás'] = report[last_y] - report[first_y]
        
        # Százalékos változás hibakezeléssel (nullával osztás -> 0)
        report['Teljes változás (%)'] = (
            (report['Teljes változás'] / report[first_y] * 100)
...
# Több adatoszlop adatainak összehasonlítása több park, 1 vagy 2 év viszonylatában
