...
def match_park_ID(park_tocheck, db_park_azonosito, threshold=70, limit=3):
    ...
    if db_park_azonosito.empty:
        return None
    
    db_park_azonosito = db_park_azonosito.reset_index(drop=True)

    # Az extractBests visszaadja a legjobb találatokat egy (szöveg, pontszám, index) hármasaként
    matches = process.extractBests(park_tocheck, db_park_azonosito['park_nev'], scorer=fuzz.token_sort_ratio, limit=limit)

    # 1. Ha nincs találat 70% felett, automatikusan None-t ad, új park_ID jön létre. 
    valid_matches = [m for m in matches if m[1] >= threshold]

    if not valid_matches:
        return None

    # 2. Ha 100%-os az egyezés, kérdés nélkül visszaadja a park_ID-t (valid_matches[0] a legjobb találat)
    best_match_text, best_score, best_index = valid_matches[0]
    
    if best_score == 100:
        return db_park_azonosito.loc[best_index, 'park_ID']
    
    # 3. Ha 70% és 99% közötti, felsorolja a találatokat és megkérdezi a felhasználót.
    print(f"\n Bizonytalan egyezés: '{park_tocheck}'")
    
    offered_ids = set()
    for match_text, score, index in valid_matches:
        p_id = db_park_azonosito.loc[index, 'park_ID']
        p_nev = db_park_azonosito.loc[index, 'park_nev']
        offered_ids.add(str(p_id))
        
        print(f" [ID: {p_id}] {p_nev} (Hasonlóság: {score}%)")
    
    print("Írd be a megfelelő ID-t, vagy '0'-át egy ÚJ PARK létrehozásához.")
    
    while True:
    
        user_input = input("Választott ID: ").strip()
        
        if user_input == "0":
            print(f"-> Új park létrehozása: '{park_tocheck}'")
            return None
        
        if user_input in offered_ids:
            return int(user_input)
    
        print("Érvénytelen ID. Kérem, válasszon a felsorolt ID-k közül, vagy írja be '0'-át új park létrehozásához.")
    


def match_cimviselo_ID(cimviselo_tocheck, db_cimviselo_azonosito, threshold=0.85):
    
    # Az extractOne visszaad egy (szöveg, pontszám, index) hármasát, ahol a legjobb találat van.
...
def transform_write_to_db(db, all_data, column_mapping):
    ...
    try:
        with sqlite3.connect(db) as conn:
            ...
            park_id = match_park_ID(park_tocheck, db_park_azonosito, threshold=70, limit=3)
            if park_id is not None:
                talalt_park_nev = db_park_azonosito.loc[db_park_azonosito['park_ID'] == park_id, 'park_nev'].values[0]
                print(f"Sikeres összekapcsolás:'{park_tocheck}' -> '{talalt_park_nev}' (park_ID: {park_id})")
            else:
                ...
                print(f"Új park hozzáadva: '{park_tocheck}') (park_ID: {park_id})")
